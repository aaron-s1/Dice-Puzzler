...
public class MazeGeneratorExtended : MonoBehaviour
{
    ...
    public bool debug_showRoof;
    public bool debug_showText;
    public bool debug_showBreadcrumb;
    ...
    public Vector3 startPos = new Vector3(1, 0, 1);
    ...
    void Spawnmaze()
    {
        foreach (Node n in NodeInfoMap)
        {
            ...
            if (n.walkable == true)
            {
                if (debug_showText == true)
                {
                    //spawn the tile info text into the scene section + distance
                    GameObject textD = Instantiate(textPrefab, new Vector3(n.gridX * tileWidth, 1, n.gridY * tileWidth), textPrefab.transform.rotation);
                    textD.GetComponent<TextMeshPro>().text = n.section.ToString() + " - " + n.distance.ToString();
                    ...
                    if (n.tilePrefab.name == "tile_tjunc_0"
                        || n.tilePrefab.name == "tile_tjunc_1a"
                        || n.tilePrefab.name == "tile_tjunc_1b"
                        || n.tilePrefab.name == "tile_tjunc_2")
                    {
                        textD.GetComponent<TextMeshPro>().faceColor = new Color32(254, 224, 0, 255);
                    }
                    if (n.tilePrefab.name == "tile_crossroads_0"
                        || n.tilePrefab.name == "tile_crossroads_1"
                        || n.tilePrefab.name == "tile_crossroads_2a"
                        || n.tilePrefab.name == "tile_crossroads_2b"
                        || n.tilePrefab.name == "tile_crossroads_3"
                        || n.tilePrefab.name == "tile_crossroads_4")
                    {
                ...
                //spawn breadcrumbs into scene, if it is a divider with door Add a wall and door prefab
                if (n.criticalPath == true)
                {
                    if (n.tileType == "tile_straight")
                    {
                        ...
                        {
                            Instantiate(dividerDoor, new Vector3(n.gridX * tileWidth, 0, n.gridY * tileWidth), Quaternion.Euler(0, n.tileRotation, 0));
                        }
                        else
                        {
                            if (debug_showBreadcrumb == true)
                            {
                                Instantiate(breadcrumb, new Vector3((n.gridX * tileWidth) + 1.0f, 3.0f, (n.gridY * tileWidth) + 1.0f), Quaternion.Euler(0, 0, 0));
                            }
                        }
                ...
                if (n.isLoop == true && n.hasDivider == true)
                {
                    if (n.tileRotation == 0)
                    {
                        Instantiate(dividerArch, new Vector3(n.gridX * tileWidth, 0, n.gridY * tileWidth), Quaternion.Euler(0, n.tileRotation, 0));
                    }
                    else
                    {
                        Instantiate(dividerWindows, new Vector3(n.gridX * tileWidth, 0, n.gridY * tileWidth), Quaternion.Euler(0, n.tileRotation, 0));
                    }
                }
                ...
                {
                    Instantiate(fakeKey, new Vector3(n.gridX * tileWidth, 0.5f, n.gridY * tileWidth), Quaternion.Euler(0, n.tileRotation, 0));
                }
        ...
        for (int i = 1; i <= NodeInfoMap.GetUpperBound(0) - 1; i++)
        {
            for (int j = 1; j <= NodeInfoMap.GetUpperBound(1) - 1; j++)
            {
                if (NodeInfoMap[i, j].walkable == false)
                {
                    ...
                    //loop tile candidates cannot have a connection to another section of the maze and cannot connect to a tile marked as door
                    ...
                    if (NodeInfoMap[(int)NorthTile.x, (int)NorthTile.y].walkable == true && NodeInfoMap[(int)NorthTile.x, (int)NorthTile.y].hasObstacle == false
                        && NodeInfoMap[(int)SouthTile.x, (int)SouthTile.y].walkable == true & NodeInfoMap[(int)SouthTile.x, (int)SouthTile.y].hasObstacle == false
